...
class FeishuAdapterSettings:
    ...
    ws_disable_native_ping: bool = False
    ws_lark_ping_interval: Optional[int] = None
...
def _run_official_feishu_ws_client(ws_client: Any, adapter: Any) -> None:
    ...
    def _apply_runtime_ws_overrides() -> None:
        try:
            ...
            # ws_lark_ping_interval controls the lark SDK's own application-layer ping.
            # ws_ping_interval controls websockets library's TCP-level ping (if not disabled).
            lark_ping = adapter._ws_lark_ping_interval if adapter._ws_lark_ping_interval is not None else adapter._ws_ping_interval
            if lark_ping is not None:
                setattr(ws_client, "_ping_interval", lark_ping)
        except Exception:
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            ws_disable_native_ping=str(
                extra.get("ws_disable_native_ping") or os.getenv("FEISHU_WS_DISABLE_NATIVE_PING", "")
            ).strip().lower() in ("true", "1", "yes"),
            ws_lark_ping_interval=_coerce_int(
                extra.get("ws_lark_ping_interval") or os.getenv("FEISHU_WS_LARK_PING_INTERVAL"),
                default=None, min_value=5,
            ),
            admins=admins,
    ...
    def _apply_settings(self, settings: FeishuAdapterSettings) -> None:
        ...
        self._ws_disable_native_ping = settings.ws_disable_native_ping
        self._ws_lark_ping_interval = settings.ws_lark_ping_interval
    ...
    async def send_exec_approval(
        ...
        If *user_id* (Feishu open_id) is provided, the card will @mention
        the user so they receive a notification in group chats.
        """
        if not self._client:
            return SendResult(success=False, error="Not connected")
    ...
    async def _add_approval_reaction(self, message_id: str, emoji_type: str) -> None:
        """Add an emoji reaction to the approval card message as click feedback."""
        if not self._client or not message_id:
            return
        try:
            from lark_oapi.api.im.v1 import (
            ...
            logger.warning("[Feishu] Failed to add approval reaction to %s", message_id, exc_info=True)
    ...
    async def _handle_card_action_event(self, data: Any) -> None:
        ...
        if hermes_action:
            ...
            # Authorization: only the user who triggered the command may resolve it
            expected_user_id = state.get("user_id", "")
            if expected_user_id and open_id != expected_user_id:
                logger.info(
                    "[Feishu] Approval %s rejected: operator %s != expected %s",
                    approval_id, open_id, expected_user_id,
                )
                # Re-insert state so the legitimate user can still act
                self._approval_state[approval_id] = state
                message_id = state.get("message_id", "")
                if message_id:
                    await self._add_approval_reaction(message_id, "FeelSorry")
                return

            choice = "once" if hermes_action == "approve_once" else "deny"
            label = "Approved" if choice == "once" else "Denied"
            ...
            message_id = state.get("message_id", "")
            await self._update_approval_card(message_id, label, user_name, choice)

            # Add emoji reaction to the approval card as immediate feedback
            if message_id:
                reaction_emoji = "THUMBSUP" if choice == "once" else "THUMBSDOWN"
                await self._add_approval_reaction(message_id, reaction_emoji)
            return
    ...
    async def _process_inbound_message(
        ...
        *,
        ...
        source = self.build_source(
            ...
            user_id=sender_profile["user_id"],
            user_name=sender_profile["user_name"],
            # 优先取飞书 SDK 的 root_id（话题根消息 ID），回退到 thread_id。
            # 若均为空（初次 @mention，话题尚未创建），对群消息用 message_id 作为
            # thread_id，使后续 thread 回复（root_id == message_id）命中同一 session。
            thread_id=(
                getattr(message, "root_id", None)
                or getattr(message, "thread_id", None)
                or (message_id if chat_type != "p2p" else None)
            ),
            user_id_alt=sender_profile["user_id_alt"],
        )
    ...
    def _should_accept_group_message(self, message: Any, sender_id: Any, chat_id: str = "") -> bool:
        ...
        if not self._allow_group_message(sender_id, chat_id):
            return False
        raw_content = getattr(message, "content", "") or ""
        # @_all is Feishu's @everyone placeholder — ignore to avoid responding to broadcast messages.
    ...
    async def _send_raw_message(
        ...
        *,
        ...
        # 飞书进 thread 必须走 reply 接口 + reply_in_thread=True。
        # reply_to 为空但 metadata["thread_id"] 存在时（tool 进度消息等），
        # 用 thread_id 作为 reply 目标，确保消息留在 thread 内。
        effective_reply_to = reply_to or (metadata or {}).get("thread_id") or None
