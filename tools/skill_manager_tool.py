...
def _skills_dir() -> Path:
    """Return the active profile's skills write directory at call time.
    ...
    patched module-level ``SKILLS_DIR`` (tests), otherwise re-resolve from the
    live profile-scoped HERMES_HOME / external_dirs on every call.
    """
    configured = Path(SKILLS_DIR)
    if configured != _SKILLS_DIR_AT_IMPORT:
        return configured
    return _get_skills_write_dir()
